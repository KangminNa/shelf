...
- 함께 실려오는 소개 페이지(`site/`)를 부팅 때 앱 목록에 유지한다
  - 저장소가 `/shelf` 로 마운트돼 있을 때만 — 없으면 조용히 넘어간다
  - 이미 있으면 **건드리지 않는다**. 사용자가 포트·도메인을 고쳤다면 그 값이 이긴다
  - 컨테이너가 한 번도 없었을 때만 배포한다. 부팅마다 다시 빌드하지 않는다
  - `SHELF_SITE=off` 면 등록 자체를 하지 않는다
...
**DeployPipeline**의 빌드 컨텍스트 규칙 — 어디서 `docker build` 할지는 `buildContext()`가 혼자 정한다.
- 기본은 저장소 루트. `build_path`가 있으면 그 하위 폴더에서 빌드한다 (모노레포)
- 저장소 **밖으로 나가는 경로는 거부**한다 — `..`도, 절대경로도, 심볼릭 링크로 빠져나가는 것도
- 금지: 화면·컨트롤러가 경로를 직접 조립하는 것. 물어보면 이 함수가 답한다.
...
**UsageSampler** — 앱별 자원 사용량의 **최근 표본 하나**를 들고 있다가, 물으면 즉시 준다.
- `docker stats`는 1초가 넘게 걸린다 — 화면이 물을 때마다 부르면 안 된다. 주기적으로 떠서 캐시한다
- 표본이 없으면 `null`을 준다 (화면이 `—`로 표시)
- 금지: 화면 요청 시점에 docker를 호출하는 것
...
- 상태 조회는 앱 수만큼이 아니라 **한 번에 묶어서** 한다 (`containers.statuses`)
